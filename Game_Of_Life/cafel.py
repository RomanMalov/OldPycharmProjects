...
def com(event):
    ...
    for x in range(20):
        for y in range(16):
            a[x, y] = 1 if ((nb[x, y] == 1) or (nb[x, y] == 6) or (nb[x, y] == 5) or (a[x, y] and (nb[x, y] == 4)) or (a[x, y] and (nb[x, y] == 3))) else 0
...
def func(evet):
    ...
    y = (evet.y//50)
    if a[x, y] == 0:
        color = '#'+'00'+('000'+str(hex(random.randint(0,256)))[2:])[-2:]+('000'+str(hex(random.randint(0,256)))[2:])[-2:]
